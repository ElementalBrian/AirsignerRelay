...
class Splash(tornado.web.RequestHandler, ABC):
    async def get(self):
        try:
            ...
            bid = f'If you\'d like to place a bid, go here: {(socket.gethostbyname(socket.gethostname()))}:{ports}/bid?key=APIKEY&endpoint=ENDPOINT&amount=AMOUNT&searcher=ADDRESS'
...
class Bid(tornado.web.RequestHandler):
    async def post(self):
        try:
            ...
            response: Dict = http_server.callback.place_bid(encoded_parameters, amount, searcher, endpoint_id, api_key)
            self.write(response)
        except Exception as e:
            self.send_error(400, reason=e)
